...
# === 必读筛选 Prompt ===
MUST_READ_SELECT_PROMPT = """从以下 {count} 条 AI 资讯中，选出 {n} 篇"必读"文章。

## 输入数据
{items_json}

## 必读标准
1. 影响力：对 AI 行业有重大影响或启示
2. 学习价值：对大学生有实际帮助（技术学习、行业认知、职业启发）
3. 新鲜度：今日/昨日新内容优先
4. 覆盖面：尽量覆盖不同板块，每板块至少 1 篇

## 返回格式
```json
{{
  "must_read_indices": [3, 7, 12, ...]
}}
```
只返回 JSON 对象，不要有其他文字。
"""
...
# === 必读详细摘要 Prompt ===
MUST_READ_DETAIL_PROMPT = """请为以下 {count} 篇"必读"文章生成详细摘要和关键要点。
...
## 输出要求
对每篇文章返回：
- `detail_summary`: 详细中文摘要（150-300 字，说清楚：背景、核心内容、为什么重要、对你有什么用）
- `key_points`: 3-5 个关键要点（中文，每条 15 字以内，提炼最核心的信息）
